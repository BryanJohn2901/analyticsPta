...
export function ProfileAnalysis({ selectedGroup, adAccountId, groupLabel }: ProfileAnalysisProps) {
  ...
  if (!hasToken) {
    return (
      <div className="flex flex-col items-center gap-4 rounded-xl border border-slate-200 bg-white p-12 text-center shadow-sm">
        <div className="flex h-12 w-12 items-center justify-center rounded-full bg-amber-100">
          <Key size={22} className="text-amber-600" />
        </div>
        <div>
          <p className="text-sm font-semibold text-slate-800">Token de acesso não configurado</p>
          <p className="mt-1 text-xs text-slate-500">
            Configure o Access Token da Meta Ads API em{" "}
            <span className="font-medium text-slate-700">Importar dados → Meta Ads API</span>
          </p>
        </div>
      </div>
    );
  ...
  if (!hasAccount || selectedGroup === "all") {
    return (
      <div className="flex flex-col items-center gap-4 rounded-xl border border-slate-200 bg-white p-12 text-center shadow-sm">
        ...
        <div>
          <p className="text-sm font-semibold text-slate-800">Selecione uma campanha</p>
          <p className="mt-1 text-xs text-slate-500">
            {selectedGroup === "all"
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white px-5 py-4 shadow-sm">
        <div>
          <h2 className="text-base font-bold text-slate-900">Análise por Perfil — {groupLabel}</h2>
          <p className="mt-0.5 text-xs text-slate-500">Ad Account: {adAccountId}</p>
        </div>
        <div className="flex items-center gap-2">
          <input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)}
            className="h-8 rounded-md border border-slate-300 px-2 text-xs text-slate-700 outline-none focus:border-blue-500" />
          <span className="text-xs text-slate-400">até</span>
          <input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)}
            className="h-8 rounded-md border border-slate-300 px-2 text-xs text-slate-700 outline-none focus:border-blue-500" />
          <button onClick={() => void load()} disabled={loading}
            className="flex h-8 w-8 items-center justify-center rounded-md border border-slate-300 bg-white text-slate-500 transition hover:bg-slate-50 disabled:opacity-60">
      ...
      {data.length > 0 && (
        <>
          ...
          <article className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            <h3 className="mb-4 text-sm font-semibold text-slate-800">Investimento por Conjunto de Anúncios</h3>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={data.slice(0, 10)} layout="vertical" margin={{ top: 0, right: 16, bottom: 0, left: 120 }}>
                  ...
                  <YAxis type="category" dataKey="name" stroke="#94a3b8" tick={{ fontSize: 10 }} width={115} />
                  <Tooltip
          ...
          {/* Adset table */}
          <article className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            <h3 className="mb-4 text-sm font-semibold text-slate-800">Detalhamento por Conjunto</h3>
          ...
          <article className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            <h3 className="mb-4 text-sm font-semibold text-slate-800">CTR por Conjunto (top 10)</h3>
            <div className="h-52">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={data.slice(0, 10)} margin={{ top: 4, right: 8, bottom: 0, left: 0 }}>
                  ...
                  <YAxis stroke="#94a3b8" tick={{ fontSize: 10 }} tickFormatter={(v) => `${v.toFixed(1)}%`} />
                  <Tooltip
                    contentStyle={{ borderRadius: 8, border: "1px solid #e2e8f0", fontSize: 12 }}
