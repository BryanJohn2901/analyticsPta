...
function CreativeRow({ rank, campaign: c, highlight, highlightValue }: CreativeRowProps) {
  return (
    <li className="flex items-start gap-3 rounded-lg border border-slate-100 bg-slate-50 p-3 dark:border-slate-700 dark:bg-slate-700/50">
      <span className={`mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-xs font-bold ${rank === 1 ? "bg-amber-100 text-amber-700" : rank === 2 ? "bg-slate-200 text-slate-600 dark:bg-slate-600 dark:text-slate-300" : "bg-slate-100 text-slate-500 dark:bg-slate-600 dark:text-slate-400"}`}>
        {rank}
      </span>
      <div className="min-w-0 flex-1">
        <p className="text-xs font-semibold text-slate-800 dark:text-slate-200">{c.campaignName}</p>
      ...
      <div className="shrink-0 rounded-md bg-blue-50 px-2 py-0.5 text-right dark:bg-blue-900/30">
        <p className="text-xs text-blue-500 dark:text-blue-400">{highlight}</p>
        <p className="text-sm font-bold text-blue-700 dark:text-blue-300">{highlightValue}</p>
      </div>
    </li>
  );
}
...
export function BestCreatives({ campaigns }: BestCreativesProps) {
  ...
  return (
    <div className="space-y-4">
      <article className="flex items-start gap-3 rounded-xl border border-blue-200 bg-blue-50 p-4 dark:border-blue-800 dark:bg-blue-900/20">
        <Info size={16} className="mt-0.5 shrink-0 text-blue-500" />
        <p className="text-xs text-blue-700 dark:text-blue-300">
          <span className="font-semibold">Proxy de criativos:</span> os dados atuais não têm o nome do criativo (imagem/vídeo) individualmente — a análise usa o nome da campanha como referência. Para ver performance por criativo real, será necessário adicionar uma coluna <code className="rounded bg-blue-100 px-1 dark:bg-blue-900/40">Nome do Criativo</code> no CSV ou conectar via API do Meta/Google Ads.
        </p>
      </article>
      ...
      <article className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
        <div className="mb-3 flex items-center gap-2">
          <ImageIcon size={17} className="text-slate-400 dark:text-slate-500" />
          <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300">Próximos passos para análise de criativos reais</h3>
        </div>
        <ul className="space-y-2 text-xs text-slate-600 dark:text-slate-400">
          <li className="flex items-start gap-2">
            <span className="mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-slate-100 text-slate-500 font-medium dark:bg-slate-700 dark:text-slate-400">1</span>
            <span><span className="font-medium">Via CSV:</span> adicione as colunas <code className="rounded bg-slate-100 px-1 dark:bg-slate-700">Nome do Criativo</code> e <code className="rounded bg-slate-100 px-1 dark:bg-slate-700">ID do Criativo</code> na planilha. O dashboard agrupará os dados por criativo automaticamente.</span>
          </li>
          <li className="flex items-start gap-2">
            <span className="mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-slate-100 text-slate-500 font-medium dark:bg-slate-700 dark:text-slate-400">2</span>
